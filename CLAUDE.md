...
- **Dependency graph**: Optional graph-based code dependency analysis with multi-backend support (File-based JSON, Neo4j local Docker, AWS Neptune). Opt-in during Workflow Planning with detailed backend configuration. Neo4j provides Cypher queries and browser visualization; Neptune supports IaC provisioning (CDK/Terraform/CloudFormation) and IAM auth. Deployment verification (9 checks) ensures graph DB health. Standalone `/aidlc-graph` utility available anytime with 9 modes: build, update, visualize, export (PNG), impact, search, repair, verify, teardown. E2E verified with Neo4j backend (15-node/41-edge TypeScript project, all verification checks passed) and Neptune backend (185-node/403-edge 10-unit monorepo, GraphRAG semantic search verified). Note: use MERGE (not CREATE) for idempotent Cypher operations; cypher-shell has JVM cold-start overhead (~1s) separate from actual query latency. Integrated pipeline: RE builds initial graph (brownfield), Code Gen updates incrementally per unit with post-update verification, Build & Test uses impact analysis for prioritized test execution (P1 direct, P2 1-hop, P3 full suite) with E2E verification checklist. Graph DB lifecycle (init, health checks, parallel coordination, teardown) managed by orchestrator. Graph operations are non-blocking — failures log warnings and continue the workflow. All graph node IDs follow a strict normalization convention: `u-{NN}` for units (lowercase, zero-padded), `mod-{kebab-name}` for modules, `file-{kebab-name}` for files, `comm-{kebab-name}` for communities, `summary-{parent-id}` for summaries — always lowercase, hyphen-separated, type-prefixed. IMPORTS edges follow standardization rules: one edge per import statement, internal files only, resolve to file IDs, deduplicate with MERGE. PNG export generates three visualization types (full graph, community architecture, impact analysis) via Python networkx+matplotlib. Neptune openCypher does not support `=~` regex — use STARTS WITH, ENDS WITH, or CONTAINS instead. Neptune connectivity via SSM port forwarding may fail with WebSocket timeout — use SSM Run Command as fallback. Neptune bastion SG requires self-referencing ingress on port 443 for SSM VPC Endpoint communication — without this the SSM agent stays in PingStatus: None indefinitely. Neptune CloudFormation teardown may be blocked by orphaned VPC Endpoint ENIs and GuardDuty-managed security groups — delete these manually before retrying stack deletion. Graph-enabled projects include graph infrastructure items in the Operations deployment checklist and developer README.
...
- **CGIG**: Optional Compilation-Guided Iterative Graph-retrieval for automated compilation error repair. Opt-in during Workflow Planning (Tier 2.25) with 4 graph construction methods: Static (default, full AST), CGIG (class-level enriched with constructors/methods/fields/type hierarchy), Lightweight (import-only, minimal overhead), Hybrid (static base + reactive CGIG on failure). When enabled, Build & Test runs a compile→parse→graph-query→fix loop (configurable rounds and confidence threshold). 10 language-agnostic error categories for classification. Repair mode available as 9th graph-analyzer mode. All CGIG operations are non-blocking.
